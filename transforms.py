...
from __future__ import division # 导入未来支持的语言 division是精确除法 相见https://blog.csdn.net/feixingfei/article/details/7081446
import torch
import math
import random

from PIL import Image, ImageOps, ImageEnhance
try: # 尝试做...如果不行则...  python中所有含条件的语句都要加冒号
    import accimage
except ImportError:
    accimage = None
...
def _is_numpy_image(img): # numpy格式的图像
    return isinstance(img, np.ndarray) and (img.ndim in {2, 3})
    # return ... and ... 和 return ... or ...
    # 详见https://zhidao.baidu.com/question/1642792031752755300.html
    # 详见http://www.mamicode.com/info-detail-1765166.html

def _is_pil_image(img): # PIL格式的图像
    if accimage is not None:
        return isinstance(img, (Image.Image, accimage.Image))
    else:
        return isinstance(img, Image.Image)

def _is_tensor_image(img): # tensor格式的图像
...
def adjust_brightness(img, brightness_factor):
    ...
    if not _is_pil_image(img):
        # 当程序出现错误时，python会自动引发异常，也可以通过raise显示地引发异常。
...
def adjust_hue(img, hue_factor): # hue: 色调
...
class Compose(object):
    ...
    # __call__：把类的实例当成函数来用，详见https://blog.csdn.net/yaokai_assultmaster/article/details/70256621
...
class NormalizeNumpyArray(object):
    ...
    def __call__(self, img):
        ...
        if not(_is_numpy_image(img)):
            raise TypeError('img should be ndarray. Got {}'.format(type(img)))
        # TODO: make efficient
        # TODO在python中是一种助记符(Mnemonics)，用来解释将要做什么。
        print(img.shape)
        for i in range(3): # 对RGB图像的每个通道进行同样的操作
...
class NormalizeTensor(object):
    ...
    def __call__(self, tensor):
        ...
        # 详见http://www.runoob.com/python/python-func-zip.html
...
class CenterCrop(object):
    ...
    # @staticmethod详见https://www.zhihu.com/question/20021164
    @staticmethod
...
class ColorJitter(object): # jitter：抖动
